        ...
        <>
            ...
            <div className='flex flex-col md:flex-row gap-8 mt-4 px-4'>
                ...
                <div className='flex flex-wrap gap-4 w-full md:w-3/4'>
                    {limitedData.map((e, i) => (
                        <div key={i} className="p-2 w-full sm:w-1/2 md:w-1/3 lg:w-1/4 relative group">
                            <img src={Images[e.image]} alt={e.title} className="w-full h-auto transition-transform duration-300 group-hover:scale-105" />
                            <h1 className='cursor-pointer' onClick={() => { navigate(`/details/${e.id}`) }}>{e.title}</h1>
                            <p>{e.price}</p>
                            <p>{e.type}</p>
                            <button
                                className='absolute bottom-2 right-2 bg-black text-white px-4 py-2 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-300'
                                onClick={() => addToCart(e)}
                            >
                                Add to Cart
                            </button>
